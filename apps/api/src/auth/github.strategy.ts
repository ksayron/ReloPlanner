...
export class GithubStrategy extends PassportStrategy(Strategy, 'github') {
  ...
  constructor(
    ...
    const configuredClientId =
      config.get<string>('GITHUB_OAUTH_CLIENT_ID') ?? '';
    const configuredClientSecret =
      config.get<string>('GITHUB_OAUTH_CLIENT_SECRET') ?? '';
    ...
    super({
      clientID: clientId,
      clientSecret,
      callbackURL: auth.getGithubCallbackUrl('auth'),
      scope: ['user:email'],
  ...
  async validate(
    ...
    return this.auth.resolveGithubCallback(state, profile);
  }
}
